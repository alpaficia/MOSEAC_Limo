...
class NatNetClient(object):
    ...
    def run(self):
        ...
        self.__sendCommand(self.NAT_PING, "", self.commandSocket, (self.server_address, self.command_port))

        self.__sendCommand(self.NAT_REQUEST_MODELDEF, "", self.commandSocket, (self.server_address, self.command_port))

    def close(self):
        self.__sendCommand(self.NAT_DISCONNECT, "", self.commandSocket, (self.server_address, self.command_port))
        with self._lock:
            self._is_alive = False
...
if __name__ == '__main__':
    ...
    parser.add_argument("--record2file", help="Record rigidbody data to a file (dictionary using pickle).",
                        action="store_true")
