...
time_now = '_' + time_now[0:10] + '_' + time_now[-5::]
save_path = '/model/SEAC_time{}'.format("dynamicworld-nodelay") + time_now
load_path = '/model/'
dir_path = os.getcwd()
...
class SEACAgent(object):
    ...
    def save(self, episode):
        torch.save(self.actor.state_dict(), save_path + "/seac_actor{}.pth".format(episode))
        torch.save(self.q_critic.state_dict(), save_path + "/seac_q_critic{}.pth".format(episode))
        print("current models has been saved")

    def load(self, episode):
        self.actor.load_state_dict(torch.load(load_path + "/seac_actor{}.pth".format(episode)))
        self.q_critic.load_state_dict(torch.load(load_path + "/seac_q_critic{}.pth".format(episode)))
